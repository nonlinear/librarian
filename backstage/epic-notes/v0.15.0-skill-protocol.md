...
flowchart TB
    ...
    CHECK -->|NO| ERROR(["*🤚 No metadata found:<br>Run librarian index* 7️⃣"])
    CHECK --> INFER{"Infer scope? 2️⃣"}
    
    INFER -->|"confidence lower than 75%"| CLARIFY(["*🤚 Say it again?* 7️⃣"])
    INFER --> BUILD(["Build command 3️⃣"])
    
    CLARIFY --> TRIGGER
    
    BUILD --> CHECK_SYSTEM{"System working?"}
    
    CHECK_SYSTEM -->|NO| BROKEN(["*🤚 System is broken* 7️⃣"])
    CHECK_SYSTEM --> EXEC(["Run python script with flags"])
    
    EXEC --> JSON(["Return JSON"])
    JSON --> CHECK_RESULTS{"Results found?"}
    
    CHECK_RESULTS -->|NO| EMPTY(["*🤚 No results found* 7️⃣"])
    CHECK_RESULTS --> FORMAT(["Format output 5️⃣"])
