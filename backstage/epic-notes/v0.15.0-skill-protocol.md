...
flowchart TB
    ...
    TRIGGER --> METADATA(["Load metadata<sup>1</sup>"])
    METADATA --> CHECK{"Metadata exists?"}
    
    CHECK -->|"No"| ERROR(["*🤚 No metadata found:<br>Run librarian index*<sup>5</sup>"])
    CHECK -->|"Yes"| INFER{"Infer scope?<sup>2</sup>"}
    
    INFER -->|"confidence<br>lower than 75%"| CLARIFY(["*🤚 Say it again?*<sup>5</sup>"])
    INFER -->|"confidence<br>higher than 75%"| BUILD(["Build command<sup>3</sup>"])
    
    BUILD --> CHECK_SYSTEM{"System working?"}
    
    CHECK_SYSTEM -->|"No"| BROKEN(["*🤚 System is broken*<sup>5</sup>"])
    CHECK_SYSTEM -->|"Yes"| EXEC(["Run python script with flags"])
    
    EXEC --> JSON(["Return JSON"])
    JSON --> CHECK_RESULTS{"Results found?"}
    
    CHECK_RESULTS -->|"No"| EMPTY(["*🤚 No results found*<sup>5</sup>"])
    CHECK_RESULTS -->|"Yes"| FORMAT(["Format output<sup>4</sup>"])
