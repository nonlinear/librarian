...
## Solution: Move logic from SKILL.md to shell scripts

**Architecture (Runtime Flow - Claw Implementation):**

```mermaid
flowchart TD
    USER["🟨 User says<br/>'pesquisa X'"] --> DETECT["🟨 I detect trigger<br/>✅ Works"]
    DETECT --> PARSE["🔴 Parse scope<br/>1️⃣ Broken"]
    PARSE --> RUN["🔵 librarian.sh<br/>✅ Created"]
    RUN --> PY["🔵 librarian.py<br/>✅ Created"]
    PY --> RESEARCH["🔵 research.py<br/>✅ Exists"]
    RESEARCH --> CHECK{"🔵 Empty results?<br/>✅ Implemented"}
    CHECK -->|Yes| EMPTY["🔵 Return 'não achei'<br/>✅ No invention"]
    CHECK -->|No| FORMAT["🔵 Format citations<br/>✅ 1️⃣ 2️⃣ 3️⃣"]
    FORMAT --> OUTPUT["🔵 Show AS-IS<br/>✅ Zero interpretation"]
    
    style USER fill:#FFC107,stroke:#F57C00,color:#000
    style DETECT fill:#FFC107,stroke:#F57C00,color:#000
    style PARSE fill:#f44336,stroke:#c62828,color:#fff
...
**Protocol Flow (Sandwich Architecture - Design):**
