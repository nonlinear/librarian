...
:root {
  ...
  --developed-plus-bg: #FFE082;
  --developed-plus-border: #F57C00;
  --partial-bg: #E1BEE7;
...
.node.approved {
  ...
  border: 2px solid var(--approved-border);
  color: #000;
}

.node.blocker {
  background-color: var(--blocker-bg);
  border: 2px solid var(--blocker-border);
  color: #000;
}

.node.developed {
  background-color: var(--developed-bg);
  border: 2px solid var(--developed-border);
  color: #000;
}

.node.developed-plus {
  background-color: var(--developed-plus-bg);
  border: 2px solid var(--developed-plus-border);
  color: #000;
}

.node.partial {
  background-color: var(--partial-bg);
  border: 2px solid var(--partial-border);
  color: #000;
}
</style>
...
```mermaid
flowchart TB
    TRIGGER(["🎤 Trigger + context<br/>✅ Works"]):::approved
    TRIGGER --> METADATA(["👷 Load metadata _1_<br/>⚫ Not implemented"]):::default
    METADATA --> CHECK{"👷 Metadata exists?<br/>⚫ Not implemented"}:::default
    
    CHECK -->|"No"| ERROR(["🎤 *🤚 No metadata found:<br>Run librarian index* _5_"]):::approved
    CHECK -->|"Yes"| INFER{"🎤 Infer scope? _2_<br/>🔴 Broken"}:::blocker
    
    INFER -->|"confidence<br>lower than 75%"| CLARIFY(["🎤 *🤚 Say it again?* _5_"]):::approved
    INFER -->|"confidence<br>higher than 75%"| BUILD(["👷 Build command _3_<br/>✅ Created"]):::developed
    
    BUILD --> CHECK_SYSTEM{"⚙️ System working?<br/>✅ Works"}:::developed
    
    CHECK_SYSTEM -->|"No"| BROKEN(["🎤 *🤚 System is broken* _5_"]):::approved
    CHECK_SYSTEM -->|"Yes"| EXEC(["⚙️ Run python script with flags<br/>✅ Implemented"]):::developed
    
    EXEC --> JSON(["⚙️ Return JSON<br/>✅ Works"]):::developed
    JSON --> CHECK_RESULTS{"👷 Results found?<br/>✅ Implemented"}:::developed
    
    CHECK_RESULTS -->|"No"| EMPTY(["🎤 *🤚 No results found* _5_<br/>✅ Implemented"]):::developed
    CHECK_RESULTS -->|"Yes"| FORMAT(["🎤 Format output _4_<br/>✅ Implemented"]):::developed
    
    FORMAT --> RESPONSE(["🎤 Librarian response<br/>✅ Works"]):::developed
    
    %% CSS Classes (must match :root variables above)
    classDef default fill:var(--default-bg),stroke:var(--default-border),stroke-width:2px,color:#000
    classDef approved fill:var(--approved-bg),stroke:var(--approved-border),stroke-width:2px,color:#000
    classDef blocker fill:var(--blocker-bg),stroke:var(--blocker-border),stroke-width:2px,color:#000
    classDef developed fill:var(--developed-bg),stroke:var(--developed-border),stroke-width:2px,color:#000
    classDef developed+ fill:var(--developed-plus-bg),stroke:var(--developed-plus-border),stroke-width:2px,color:#000
    classDef partial fill:var(--partial-bg),stroke:var(--partial-border),stroke-width:2px,color:#000
```
...
|---|------|-------|------|------------------------|
| 1 | <span style="background-color: var(--default-bg); color: black; padding: 4px 12px; border: 2px solid var(--default-border);">█</span> | <span style="background-color: var(--default-border); color: white; padding: 4px 12px;">█</span> | **default** | No agreement yet. Backlog, not discussed, not approved. |
| 2 | <span style="background-color: var(--approved-bg); color: black; padding: 4px 12px; border: 2px solid var(--approved-border);">█</span> | <span style="background-color: var(--approved-border); color: white; padding: 4px 12px;">█</span> | **approved** | Agreed by all stakeholders, ready for development. Sometimes has notes. |
| 3 | <span style="background-color: var(--blocker-bg); color: black; padding: 4px 12px; border: 2px solid var(--blocker-border);">█</span> | <span style="background-color: var(--blocker-border); color: white; padding: 4px 12px;">█</span> | **blocker** | Either needs discussion to agree, OR failed implementation. **Always with numbered note.** |
| 4 | <span style="background-color: var(--developed-bg); color: black; padding: 4px 12px; border: 2px solid var(--developed-border);">█</span> | <span style="background-color: var(--developed-border); color: white; padding: 4px 12px;">█</span> | **developed** | Agreed and implemented. Ready for testing. |
| 5 | <span style="background-color: var(--developed-plus-bg); color: black; padding: 4px 12px; border: 2px solid var(--developed-plus-border);">█</span> | <span style="background-color: var(--developed-plus-border); color: white; padding: 4px 12px;">█</span> | **developed+** | Agreed and implemented, but developer (AI) made decisions that warrant discussion. |
| 6 | <span style="background-color: var(--partial-bg); color: black; padding: 4px 12px; border: 2px solid var(--partial-border);">█</span> | <span style="background-color: var(--partial-border); color: white; padding: 4px 12px;">█</span> | **partial** | Node works, but some paths (arrows) fail. **Use arrow colors to show which paths work vs fail.** |
...
|--------|--------|------|-------------|
| `A --> B` | —→ | Normal flow | Standard sequential process (currently used) |
| `A -.-> B` | ⋯→ | Optional/conditional | Sometimes happens, edge cases, fallback paths |
| `A ==> B` | ═→ | Critical/main path | Happy path, most important flow, primary route |
| `A --- B` | — | Bidirectional/state | Shares state, no direction, mutual relationship |
| `A -->|"label"| B` | —label→ | Labeled transition | Decision outcomes, conditions, context |

**Arrow colors** (use `linkStyle N stroke:#color,stroke-width:2px`):
- Success/working path: `#64B5F6` (blue)
- Error/failure path: `#E57373` (red)
- Warning/needs attention: `#FFB74D` (yellow)
- Partial/some work: `#BA68C8` (purple)
