...
flowchart TB
    ...
    INFER -->|< 75%| CLARIFY["🎤 🤚 Say it again?"]:::green
    INFER -->|≥ 75%| BUILD["👷 Build command"]:::green
    
    BUILD --> CHECK_SYSTEM{"⚙️ System working?"}:::green
    
    CHECK_SYSTEM -->|No| BROKEN["🎤 🤚 System is broken"]:::green
    ...
    CHECK_RESULTS -->|Yes| FORMAT["🎤 Format output"]:::green
    
    FORMAT --> RESPONSE["🎤 Librarian response"]:::green
...
**Lesson learned:** Publish → Test → Fix → THEN Merge

### First Publish Attempt

**Published to ClawHub:**
- ❌ Missing research.py in parent engine/ (deleted during cleanup)
- ❌ Books symlink user-specific (broke on clone)
...
**Rule:** Publish → Test → Fix → Merge (not the other way)
...
```
parent-project/
├── engine/          ← Heavy computation
├── data/            ← Data files
└── skill/           ← Conversational layer (light)
    ├── SKILL.md
    ├── wrapper.sh   → ../engine/
    └── README.md    → "Install parent first"
```
...
- Installation: clone parent → activate skill
