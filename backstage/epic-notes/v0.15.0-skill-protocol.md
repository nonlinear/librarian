...
flowchart TB
    ...
    CHECK -->|"No"| ERROR(["*🤚 No metadata found:<br>Run librarian index* ^5"])
    CHECK -->|"Yes"| INFER{"Infer scope? ^2"}
    
    INFER -->|"confidence<br>lower than 75%"| CLARIFY(["*🤚 Say it again?* ^5"])
    INFER -->|"confidence<br>higher than 75%"| BUILD(["Build command ^3"])
    
    BUILD --> CHECK_SYSTEM{"System working?"}
    
    CHECK_SYSTEM -->|"No"| BROKEN(["*🤚 System is broken* ^5"])
    CHECK_SYSTEM -->|"Yes"| EXEC(["Run python script with flags"])
    
    EXEC --> JSON(["Return JSON"])
    JSON --> CHECK_RESULTS{"Results found?"}
    
    CHECK_RESULTS -->|"No"| EMPTY(["*🤚 No results found* ^5"])
    CHECK_RESULTS -->|"Yes"| FORMAT(["Format output ^4"])
