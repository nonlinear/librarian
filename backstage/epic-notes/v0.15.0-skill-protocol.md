...
flowchart TB
    TRIGGER(["🎤 Trigger + context<br/>✅ Works"])
    TRIGGER --> METADATA(["👷 Load metadata _1_<br/>⚫ Not implemented"])
    METADATA --> CHECK{"👷 Metadata exists?<br/>⚫ Not implemented"}
    
    CHECK -->|"No"| ERROR(["🎤 *🤚 No metadata found:<br>Run librarian index* _5_"])
    CHECK -->|"Yes"| INFER{"🎤 Infer scope? _2_<br/>🔴 Broken"}
    
    INFER -->|"confidence<br>lower than 75%"| CLARIFY(["🎤 *🤚 Say it again?* _5_"])
    INFER -->|"confidence<br>higher than 75%"| BUILD(["👷 Build command _3_<br/>✅ Created"])
    
    BUILD --> CHECK_SYSTEM{"⚙️ System working?<br/>✅ Works"}
    
    CHECK_SYSTEM -->|"No"| BROKEN(["🎤 *🤚 System is broken* _5_"])
    CHECK_SYSTEM -->|"Yes"| EXEC(["⚙️ Run python script with flags<br/>✅ Implemented"])
    
    EXEC --> JSON(["⚙️ Return JSON<br/>✅ Works"])
    JSON --> CHECK_RESULTS{"👷 Results found?<br/>✅ Implemented"}
    
    CHECK_RESULTS -->|"No"| EMPTY(["🎤 *🤚 No results found* _5_<br/>✅ Implemented"])
    CHECK_RESULTS -->|"Yes"| FORMAT(["🎤 Format output _4_<br/>✅ Implemented"])
    
    FORMAT --> RESPONSE(["🎤 Librarian response<br/>✅ Works"])
    
    style TRIGGER fill:#FFE083,stroke:#FFB74D,stroke-width:2px,color:#000
    style METADATA fill:#CECECE,stroke:#9E9E9E,stroke-width:2px,color:#000
    style INFER fill:#FAB3AE,stroke:#E57373,stroke-width:2px,color:#000
    style CLARIFY fill:#FFE083,stroke:#FFB74D,stroke-width:2px,color:#000
    style BUILD fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style EXEC fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style JSON fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style FORMAT fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style RESPONSE fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style CHECK fill:#CECECE,stroke:#9E9E9E,stroke-width:2px,color:#000
    style ERROR fill:#FFE083,stroke:#FFB74D,stroke-width:2px,color:#000
    style CHECK_SYSTEM fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style BROKEN fill:#FFE083,stroke:#FFB74D,stroke-width:2px,color:#000
    style CHECK_RESULTS fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
    style EMPTY fill:#90CAF9,stroke:#64B5F6,stroke-width:2px,color:#000
...
|---|------|-------|------|------------------------|
| 1 | <span style="background-color: #f5f5f5; color: black; padding: 4px 12px; border: 2px solid #999;">█</span> | <span style="background-color: #999; color: white; padding: 4px 12px;">█</span> | **default** | No agreement yet. Backlog, not discussed, not approved. |
| 2 | <span style="background-color: #C8E6C9; color: black; padding: 4px 12px; border: 2px solid #388E3C;">█</span> | <span style="background-color: #388E3C; color: white; padding: 4px 12px;">█</span> | **approved** | Agreed by all stakeholders, ready for development. Sometimes has notes. |
| 3 | <span style="background-color: #FFCDD2; color: black; padding: 4px 12px; border: 2px solid #D32F2F;">█</span> | <span style="background-color: #D32F2F; color: white; padding: 4px 12px;">█</span> | **blocker** | Either needs discussion to agree, OR failed implementation. **Always with numbered note.** |
| 4 | <span style="background-color: #BBDEFB; color: black; padding: 4px 12px; border: 2px solid #1976D2;">█</span> | <span style="background-color: #1976D2; color: white; padding: 4px 12px;">█</span> | **developed** | Agreed and implemented. Ready for testing. |
| 5 | <span style="background-color: #FFE082; color: black; padding: 4px 12px; border: 2px solid #F57C00;">█</span> | <span style="background-color: #F57C00; color: white; padding: 4px 12px;">█</span> | **developed+** | Agreed and implemented, but developer (AI) made decisions that warrant discussion. |
| 6 | <span style="background-color: #E1BEE7; color: black; padding: 4px 12px; border: 2px solid #7B1FA2;">█</span> | <span style="background-color: #7B1FA2; color: white; padding: 4px 12px;">█</span> | **partial** | Node works, but some paths (arrows) fail. **Use arrow colors to show which paths work vs fail.** |
