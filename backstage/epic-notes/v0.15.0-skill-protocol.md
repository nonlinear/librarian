...
|------|-----|--------|---------|
| .node | Gray | Dark gray | No agreement yet. Backlog, not discussed, not approved. |
| .approved | Yellow | Dark yellow | Agreed by all stakeholders, ready for development. Sometimes has notes. |
| .blocker | Red | Dark red | Either needs discussion to agree, OR failed implementation. **Always with numbered note.** |
| .developed | Blue | Dark blue | Agreed and implemented. Ready for testing. |
| .developed-notes | Orange | Dark orange | Agreed and implemented, but developer (AI) made decisions that warrant discussion. |
| .partial | Purple | Dark purple | Node works, but some paths (arrows) fail. **Use arrow colors to show which paths work vs fail.** |
