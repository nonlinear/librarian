...
.node {
  ...
  border: dashed 10px red !important;
}
...
```mermaid
flowchart TB
    TRIGGER(["🎤 Trigger + context<br/>✅ Works"]):::node:::developed
    TRIGGER --> METADATA(["👷 Load metadata _1_<br/>⚫ Not implemented"]):::node:::default
    METADATA --> CHECK{"👷 Metadata exists?<br/>⚫ Not implemented"}:::node:::default
    
    CHECK -->|No| ERROR(["🎤 *🤚 No metadata found:<br>Run librarian index* _5_"]):::node:::approved
    CHECK -->|Yes| INFER{"🎤 Infer scope? _2_<br/>🔴 Broken"}:::node:::blocker
    
    INFER -->|confidence lower than 75%| CLARIFY(["🎤 *🤚 Say it again?* _5_"]):::node:::approved
    INFER -->|confidence higher than 75%| BUILD(["👷 Build command _3_<br/>✅ Created"]):::node:::developed
    
    BUILD --> CHECK_SYSTEM{"⚙️ System working?<br/>✅ Works"}:::node:::developed
    
    CHECK_SYSTEM -->|No| BROKEN(["🎤 *🤚 System is broken* _5_"]):::node:::approved
    CHECK_SYSTEM -->|Yes| EXEC(["⚙️ Run python script with flags<br/>✅ Implemented"]):::node:::developed
    
    EXEC --> JSON(["⚙️ Return JSON<br/>✅ Works"]):::node:::developed
    JSON --> CHECK_RESULTS{"👷 Results found?<br/>✅ Implemented"}:::node:::developed
    
    CHECK_RESULTS -->|No| EMPTY(["🎤 *🤚 No results found* _5_<br/>✅ Implemented"]):::node:::developed
    CHECK_RESULTS -->|Yes| FORMAT(["🎤 Format output _4_<br/>✅ Implemented"]):::node:::developed
    
    FORMAT --> RESPONSE(["🎤 Librarian response<br/>✅ Works"]):::node:::developed
```
