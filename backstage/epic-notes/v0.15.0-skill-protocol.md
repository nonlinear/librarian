...
flowchart TD
    ...
    CHECK -->|NO| ERROR(["*⚠️ No metadata found:<br>Run librarian index*<br>🤚 HARD STOP 4️⃣"])
    
    INFER -->|"confidence higher than 75%"| BUILD(["Build command 3️⃣"])
    INFER -->|"confidence lower than 75%"| CLARIFY(["*Say it again?*<br>🤚 HARD STOP 6️⃣"])
    CLARIFY --> TRIGGER
    
    BUILD --> CHECK_SYSTEM{"System working?"}
    CHECK_SYSTEM -->|YES| EXEC(["Run python script with flags"])
    CHECK_SYSTEM -->|NO| BROKEN(["*System is broken*<br>🤚 HARD STOP"])
    
    EXEC --> JSON(["Return JSON"])
    JSON --> CHECK_RESULTS{"Results found?"}
    CHECK_RESULTS -->|YES| FORMAT(["Format output 5️⃣"])
    CHECK_RESULTS -->|NO| EMPTY(["*No results found for this query*<br>🤚 HARD STOP"])
