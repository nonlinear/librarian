...
flowchart TB
    ...
    CHECK --> ERROR(["🎤 *🤚 No metadata found:<br>Run librarian index* _5_"])
    CHECK --> INFER{"🎤 Infer scope? _2_<br/>🔴 Broken"}
    
    INFER --> CLARIFY(["🎤 *🤚 Say it again?* _5_"])
    INFER --> BUILD(["👷 Build command _3_<br/>✅ Created"])
    
    BUILD --> CHECK_SYSTEM{"⚙️ System working?<br/>✅ Works"}
    
    CHECK_SYSTEM --> BROKEN(["🎤 *🤚 System is broken* _5_"])
    CHECK_SYSTEM --> EXEC(["⚙️ Run python script with flags<br/>✅ Implemented"])
    
    EXEC --> JSON(["⚙️ Return JSON<br/>✅ Works"])
    JSON --> CHECK_RESULTS{"👷 Results found?<br/>✅ Implemented"}
    
    CHECK_RESULTS --> EMPTY(["🎤 *🤚 No results found* _5_<br/>✅ Implemented"])
    CHECK_RESULTS --> FORMAT(["🎤 Format output _4_<br/>✅ Implemented"])
