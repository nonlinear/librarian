...
- ✅ `skill/research.py` included (21KB)
- ✅ Smart path detection (works standalone OR with parent)
- ✅ `skill/setup.sh` downloads model + installs deps
- ✅ `skill/requirements.txt` lists Python dependencies

**What this means for v0.21.0:**
- research.py already portable ✅
- Model download already automated (setup.sh) ✅
- **Still need:** Onboarding (where are books?), embedded indexing, multi-instance config

**Skill structure (v0.15.0):**
```
skill/
├── research.py          ← Search engine (standalone)
├── librarian.sh         ← Wrapper (smart paths)
├── setup.sh             ← Model download + deps
├── requirements.txt     ← Python deps
├── books/ (symlink)     ← User books (or parent)
└── models/ (gitignored) ← Downloaded on setup
```
...
Skill: "✅ Config saved. Run 'update librarian' to index."
```
...
Skill: 
  ...
  - Updates metadata
  - Reports: "✅ Indexed 5 new books, updated 2 topics"
```
...
> [Síntese] 📕 [Book, p.21, ¶2]
>
> [Mais detalhes] 📕 [Book, p.40, ¶5]
>
> **Sources:**
> - 📕 Book Title (Author) - X passages
> - Similarity: ⭐⭐⭐⭐⭐
```

**Proposed format:**
```markdown
> 1️⃣ [First point] 📕 [Book, p.21, ¶2]
>
> 2️⃣ [Second point] 📕 [Book, p.40, ¶5]
>
> 3️⃣ [Third point] 📕 [Book, p.89, ¶1]
>
> **Sources:**
> - 📕 [Book Title](file://~/Documents/books/book.epub) - 3 passages
> - Similarity: ⭐⭐⭐⭐⭐
```

**Changes:**
- [ ] Add numbered emojis (1️⃣2️⃣3️⃣) to structure multi-point answers
- [ ] Add clickable file:// links to source books
- [ ] Update SKILL.md example output
- [ ] Test readability (does numbering help or clutter?)

**Why:**
- Easier to reference specific points in conversation ("sobre o 2️⃣...")
...
**Onboarding:**
- ✅ New user runs `setup librarian`, answers questions, gets working config
- ✅ Config validation catches errors (missing paths, invalid models)
- ✅ Can reconfigure without breaking existing indexes

**Embedded indexing:**
- ✅ `update librarian` indexes new books automatically
- ✅ Smart detection (only changed files)
- ✅ User sees progress (not black box)
- ✅ Errors handled gracefully (corrupt book = skip, report)

**Per-instance variables:**
- ✅ Can run multiple libraries on same machine
- ✅ No conflicts between instances
- ✅ Clear which instance is active
- ✅ Easy to switch instances
...
- Watch folder for new books → auto-index
- Cron job → daily smart update
- Pre-query check → index stale? update first
...
**Next steps:**
1. Design config schema (onboarding questions → JSON)
...
**Problema:** Skill com research.py incluído = pesado (torch 500MB+, models 130MB+)

**Opção A: Skill leve (companion only)**
- SKILL.md + wrappers (4 arquivos, <50KB)
- Depende de parent librarian (engine/)
- `clawhub install` NÃO funciona sozinho
- Pros: Leve, single source of truth
- Cons: Precisa parent instalado

**Opção B: Skill pesado (standalone)**
- SKILL.md + wrappers + research.py + requirements.txt + setup.sh
- Funciona sozinho (self-contained)
- `clawhub install` funciona ✅
- Pros: Standalone, fácil instalar
- Cons: Duplicação (500MB+ deps), manutenção de 2 cópias

**Status atual (v0.15.0):** Opção B (research.py incluído)

**Decisão:** PENSAR v0.21.0 (deixar como está por enquanto)

**Teste:** ✅ PASSOU (funciona com research.py no skill/)
