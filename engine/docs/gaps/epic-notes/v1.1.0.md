...
```
librarian/
  books/              ← User content (stays at root)
  engine/models/             ← Move to engine/
  engine/scripts/            ← Move to engine/
  .github/            ← Stays at root (Git convention)
  .vscode/            ← Stays at root (VS Code convention)
  README.md           ← Stays at root
  requirements.txt    ← Move to engine/
  project.code-workspace ← Stays at root
```
...
```
librarian/
  books/              ← User content (ONLY thing at root besides config)
  engine/             ← Everything else
    engine/models/
    engine/scripts/
    docs/
    requirements.txt
  .github/            ← Git config (stays)
  .vscode/            ← VS Code config (stays)
  README.md           ← Entry point (stays)
  project.code-workspace ← Workspace config (stays)
```
...
- [ ] Move `engine/models/` → `engine/engine/models/`
- [ ] Move `engine/scripts/` → `engine/engine/scripts/`
- [ ] Move `requirements.txt` → `engine/requirements.txt`
