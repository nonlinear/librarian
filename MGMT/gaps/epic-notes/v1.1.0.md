...
- Renaming CONTRIBUTING→POLICY better reflects dual-level system (project + global)
...
librarian/
  engine/
    docs/             ← Status files (confusing location)
      CHANGELOG.md
      ROADMAP.md
      CONTRIBUTING.md ← Misleading name
      CHECKS.md
  .github/
    prompts/
      whatsup.prompt.md      ← Generic names
...
```
librarian/
  MGMT/               ← All meta-documentation & status files
    CHANGELOG.md
    ROADMAP.md
    POLICY.md         ← Renamed from CONTRIBUTING
    POLICY-global.md  ← Universal rules (cross-project)
    CHECKS.md
    gaps/
      epic-notes/
  .github/
    prompts/
      MGMT-start.prompt.md ← Renamed from whatsup
      MGMT-end.prompt.md   ← Renamed from wrap-it-up
      research.prompt.md
```
...
```
books/AI/policy/
  chunks.json         ← Visible clutter
  faiss.index         ← Visible clutter
  topic-index.json    ← User metadata (stays visible)
  book1.epub          ← User content
```
...
books/AI/policy/
  topic-index.json    ← User metadata (stays visible)
  .chunks.json        ← Hidden (implementation detail)
  .faiss.index        ← Hidden (implementation detail)
  book1.epub          ← User content
```

**Rationale:**

- Object files (chunks, indices) are implementation details → hide them
- Metadata (topic-index.json) is user-relevant → keep visible
...
### Part 1: MGMT System Restructure ✅

- [x] Create `MGMT/` directory structure
- [x] Copy `engine/docs/*` → `MGMT/`
...
- [x] Update prompt content (CONTRIBUTING→POLICY references)
...
- [ ] Research queries work (`python3.11 engine/engine/scripts/research.py "test" --topic ai`)
- [ ] Indexing works (`python3.11 engine/engine/scripts/indexer_v2.py --topic cooking`)
...
- Renamed CONTRIBUTING→POLICY (better reflects universal applicability)
- Renamed prompts for consistency with MGMT naming

**MGMT System Restructure (completed):**

- ✅ Created MGMT/ folder structure
- ✅ Copied engine/docs/\* to MGMT/
- ✅ Renamed CONTRIBUTING.md → POLICY.md (+ updated title)
- ✅ Created POLICY-global.md placeholder
- ✅ Updated README.md navigation (POLICY shows both project/global links)
- ✅ Updated all MGMT file navigation blocks (CHANGELOG, ROADMAP, CHECKS, POLICY)
- ✅ Renamed whatsup.prompt.md → MGMT-start.prompt.md
- ✅ Renamed wrap-it-up.prompt.md → MGMT-end.prompt.md
- ✅ Updated all CONTRIBUTING→POLICY references in both prompts
- ✅ Updated prompt titles, version numbers, and key principles
- ✅ Updated this epic notes file to reflect new scope
