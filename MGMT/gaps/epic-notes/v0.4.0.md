...
- PDF citations → `Book.pdf#page=42`
- EPUB citations → chapter/CFI references
...
- Notes location: `MGMT/gaps/epic-notes/v0.X.0.md`
...
```markdown
1️⃣ [Book.pdf (page 42)](books/topic/Book.pdf#page=42)
...
| ------------------- | ------------------------------------------ | ------ | ------ | ---------------------------------- |
| **Baseline**        | `[file.epub](path/file.epub)`              | ✅ YES | ✅ YES | Standard pill validation works     |
| **Hash Fragment**   | `[file.epub](path/file.epub#chapter3)`     | ❌ NO  | ❌ NO  | Fragment breaks pill completely    |
| **Query String**    | `[file.epub](path/file.epub?chapter=3)`    | ❌ NO  | ❌ NO  | Nothing renders at all             |
| **URL Encoding**    | `[file.epub](path/file.epub%23chapter3)`   | ❌ NO  | ❌ NO  | Nothing renders at all             |
| **Double Encoding** | `[file.epub](path/file.epub%2523chapter3)` | ❌ NO  | ❌ NO  | Nothing renders at all             |
| **Empty Hash**      | `[file.epub](path/file.epub#)`             | ✅ YES | ✅ YES | Pill works but no navigation value |

**Key Findings:**

1. **Pill validation** requires exact path matching: `workspace-relative-path/filename.ext`
2. **Any fragment** (`#anything`) breaks validation → no pill, no link rendered
3. **Alternative syntaxes** (query strings, encoding) also fail → markdown doesn't render at all
4. **Empty fragment** (`#`) preserves pill but provides zero navigation benefit

**Impact on Epic:**

🔴 **CRITICAL BLOCKER** - Cannot achieve primary goal (clickable navigation to specific pages/chapters)
...
1. ❌ **files.exclude blocking autocomplete** → Added books-specific patterns, still failed
2. ❌ **search.exclude blocking autocomplete** → Only affects search, not autocomplete
3. ❌ **Need VS Code reload** → Reloaded, still failed
4. ✅ **Git ignore patterns block autocomplete** → ROOT CAUSE FOUND
...
"files.exclude": {
  ...
  "books/**/*.pkl": true,      // ← Added redundantly
  "books/**/*.index": true     // ← Added redundantly
}
```
...
#                     ^^^^^^^^ ← PROBLEM: This ignores ALL subdirs
```
...
books/**/*.epub  # ✅ Correct: ignores only EPUB files
books/**/        # ❌ WRONG: ignores ALL folders inside books/
```
...
books/**/*.epub  # ✅ Only ignores .epub files, folders remain visible
```
...
2. **files.exclude ≠ autocomplete visibility** (only affects Explorer)
3. **search.exclude ≠ autocomplete visibility** (only affects search)
...
"files.watcherExclude": {
  "**/*.pkl": true,
  "**/*.index": true,
  "books/**/*.epub": true  // ← Performance: don't watch large files
}
```

**Status:** ✅ RESOLVED - Autocomplete should work after VS Code reload
...
✅ **Confirmed working** - Knowledge validation complete.
